-- =====================================================
-- SCRIPT: Crear tablas faltantes del sistema de datos completos
...
SELECT 
    ...
    dd.areas_investigacion,
    dd.nivel_ingles,
    dd.disponibilidad_horaria,
    COUNT(DISTINCT dc.curso_id) AS total_cursos_asignados
FROM usuarios u
...
SELECT 
    ...
    ad.horario_atencion,
    ad.ubicacion_oficina,
    COUNT(DISTINCT am.modulo_id) AS total_modulos_asignados
FROM usuarios u
LEFT JOIN admin_datos ad ON u.id = ad.admin_id
LEFT JOIN usuarios supervisor ON ad.supervisor_id = supervisor.id
LEFT JOIN admin_modulos am ON u.id = am.admin_id
...
SELECT 'Script ejecutado correctamente. Tablas creadas.' AS Resultado;
